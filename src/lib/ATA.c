...
static bool ata_identify(uint16_t io, uint16_t ctrl, uint8_t slave,
                         ATA_Device *out) {
  ...
  while (!(inb(io + ATA_REG_STATUS) & ATA_SR_DRQ)) {
    if (inb(io + ATA_REG_STATUS) & ATA_SR_ERR)
      return false;
  }
...
void ata_pio_read_sector(uint8_t device_index, uint32_t lba, uint8_t *buffer) {
  if (device_index >= ata_device_count)
    return;
...
uint8_t ata_get_device_count() { return ata_device_count; }
