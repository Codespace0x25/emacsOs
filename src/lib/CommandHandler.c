...
#define REGISTER_COMMAND(name, help_text, func, ManPage)                       \
  {name, help_text, func, ManPage}
...
static Command commands[] = {
    ...
    REGISTER_COMMAND("music", "plays a tuen", cmd_music,
                     "play music form a list of songs using beeper. must have "
                     "a beeper on the pc"),
    REGISTER_COMMAND("sleep", "adds a delay bases in secints", cmd_sleep,
                     "sleeps for amount of secenits set by arg 1"),
    REGISTER_COMMAND(
        "color", "sets the color.", cmd_setColor,
        "the dos command for color. works just like it dose in windows"),
    REGISTER_COMMAND("man", "the comman unix utily", cmd_man,
                     "alows one to find help for each commadn in more detail. "
                     "comman in unix"),
    REGISTER_COMMAND("memdump", "Dumps memory from a hex address", cmd_memdump,
                     "Usage: memdump <hex address> <length>\nPrints memory "
                     "content starting at the given address."),
...
void cmd_memdump(int argc, char **argv) {
  if (argc < 3) {
    printf("Usage: memdump <hex address> <length>\n");
    return;
  }

  uint32_t addr = (uint32_t)hex_string_to_int(argv[1]);
  size_t length = string_to_int(argv[2]);
